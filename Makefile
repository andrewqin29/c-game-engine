...
STUDENT_LIBS = asset asset_cache collision sdl_wrapper quiz_bank

EMCC_FLAGS = -s USE_SDL_MIXER=2  -s SDL2_MIXER_FORMATS='["mp3","wav"]' --preload-file assets --preload-file assets/fonts@/assets/fonts
...
EMCC = emcc
EMCC_FLAGS = -s EXIT_RUNTIME=1 -s ALLOW_MEMORY_GROWTH=1 -s INITIAL_MEMORY=655360000 -s USE_SDL=2 -s USE_SDL_GFX=2 -s USE_SDL_IMAGE=2 -s SDL2_IMAGE_FORMATS='["png"]' -s USE_SDL_TTF=2  -s USE_SDL_MIXER=2 -s SDL2_MIXER_FORMATS='["mp3","wav"]'  -s ASSERTIONS=1 -O2 -g -gsource-map  --use-preload-plugins --preload-file assets --preload-file assets/fonts@/assets/fonts --source-map-base http://labradoodle.caltech.edu:$(shell cs3-port)/bin/
